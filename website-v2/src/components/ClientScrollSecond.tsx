const ClientScrollSecond = () => {
  ...
  return (
    <section className="w-full py-lg bg-gray-50 overflow-hidden">
      <div className="flex animate-scroll-fast space-x-2xl items-center">
        {/* First set */}
        {clients.map((client, index) => (
          <div key={`first-${index}`} className="flex-shrink-0">
            <img src={client.logo} alt={client.name} className="h-10 opacity-50 hover:opacity-80 transition-opacity" />
          </div>
        ))}
...
export default ClientScrollSecond; 